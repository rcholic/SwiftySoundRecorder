...
//  SwiftySoundRecorder
//
//  Created by Guoliang Wang on 8/16/16.
//  ------->  == Code shameless stolen from ImagePicker on Github.com == <-------
//
...
public class AssetManager {
    public static func getImage(name: String?) -> UIImage {
        guard let imageName = name else {
            ...
            let noImage = UIGraphicsGetImageFromCurrentImageContext()
        ...
        if let bundlePath = bundle.resourcePath?.stringByAppendingString("/SwiftySoundRecorder.bundle"), resourceBundle = NSBundle(path: bundlePath) {
            bundle = resourceBundle
        }
        
        return UIImage(named: imageName, inBundle: bundle, compatibleWithTraitCollection: traitCollection) ?? UIImage()
    }
